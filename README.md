...
# 清除所有标定数据，恢复出厂状态
# ⚠️ 执行前请先运行 examples/06_demo_recover_calibration.py 备份
```
...
存储于 `actual.moduluscali.moduluscali.csv` 和 `examples/06_demo_recover_calibration.py`。
...
| 文件 | 功能 | 运行方式 |
|------|------|---------|
| 01_demo_quickstart.py | 连接设备，读取设备信息和全部配置寄存器 | 运行后自动退出 |
| 02_demo_calibration.py | 演示单点标定、多拟合点全部标定、清除标定三种流程 | 运行后自动退出 |
| 03_demo_configuration.py | 读取并修改压力值类型、AD 屏蔽值、设备地址等参数 | 运行后自动退出 |
| 04_demo_read_pressure.py | 以 200 Hz 精确控制采样，每秒统计实际帧率 | Ctrl+C 停止 |
| 05_demo_record_pressure.py | 交互式设置采样率和时长，将数据保存为 CSV | 含交互提示 |
| 06_demo_recover_calibration.py | 将出厂标定数据一键写回设备 | 运行后自动退出 |
| 07_demo_test_fps.py | 全速采集压测，测量当前硬件条件下最大 FPS | Ctrl+C 停止 |
| 08_demo_zero_calibration.py | 演示上电自动归零、手动动态归零、重置归零 | 含交互提示 |
