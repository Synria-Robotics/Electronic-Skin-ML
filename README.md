...
**注意**：
- 上电自动归零对应的寄存器为**只写**，设置后无法读取验证，需重启设备才能观察效果。
- 手动动态归零通过 Python 层软件基线实现，每个压力点独立补偿（`output[i] = max(0, raw[i] - baseline[i])`），执行前建议确保传感器表面无负载。
...
| 方法 | 说明 | 注意 |
|------|------|------|
| `config.set_auto_zero_enable(True/False)` | 上电自动归零开关 | 只写寄存器，重启后生效 |
