...
**行为说明**：将以下 11 点出厂标定表（来源：`actual.moduluscali.moduluscali.csv`）以**全部标定模式**写回设备，覆盖当前所有标定数据：
...
| 操作 | 说明 | 生效时机 |
|------|------|---------|
| 上电自动归零 | 每次上电时自动将当前压力值归零（可启用/禁用） | 下次重启后生效 |
| 手动动态归零 | 立即将当前压力值清零，执行前需确保传感器无负载 | 立即生效 |
| 重置动态归零 | 撤销之前的动态归零，恢复到出厂零点 | 立即生效 |

**注意**：上电自动归零对应的寄存器为**只写**，设置后无法读取验证，需重启设备才能观察效果。
