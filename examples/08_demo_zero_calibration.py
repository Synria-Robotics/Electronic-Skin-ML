...
【功能】
    ...
    演示1 — 上电自动归零
        固件强制行为：设备每次上电时自动将当前压力值归零，无需也无法通过软件启用或禁用。
        本演示仅作说明，无需操作。
...
def main() -> None:
    with TactilePressureSDK(port=PORT, slave_address=SLAVE_ADDRESS) as sdk:
        ...
        print("=" * 60)
        print()
        print("说明：固件强制行为，设备每次上电时自动将当前压力值归零。")
        print("      此功能始终启用，无需也无法通过软件控制开关。\n")
        print("✓ 无需操作，上电自动归零已固件层强制生效\n")
